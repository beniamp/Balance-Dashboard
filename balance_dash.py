...
ending_range = st.selectbox('Ending', price_ranges, index=len(price_ranges) - 2)
...
df_grouped = df_joined.groupby(['ProductS']).agg({'Availability': 'max', 'PriceRangeS': 'max'}).reset_index().sort_values(by='PriceRangeS', ascending=False)
